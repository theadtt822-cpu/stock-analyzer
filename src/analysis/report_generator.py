...
class ReportGenerator:
    ...
    def analyze_with_ai(self) -> dict:
        """调用 OpenClaw Gateway 本地 API 进行 AI 分析"""
        close = self._get('close')
        prev = self._get_prev('close')
...
- MACD: DIF={dif:.2f}, DEA={dea:.2f}
- RSI(14): {rsi:.1f}
- KDJ: K={k:.0f}, D={d:.0f}
- 布林带: 上轨={upper:.2f}, 下轨={lower:.2f}
- ATR: {atr:.2f}
- 量比: {vol_ratio:.1f}

请返回JSON（不要有其他文字）：
{{"operation_advice":"买入/增持/持有/观望/减仓/卖出","sentiment_score":0-100整数,"analysis_summary":"一句话核心观点20字内","target_price":数字,"stop_loss":数字,"confidence_level":0-100整数,"key_points":["关键点1","关键点2","关键点3"]}}"""
        ...
        try:
            ...
            resp = requests.post(
                ...
                json={
                    "model": "openclaw",
                    "messages": [
                        {"role": "system", "content": "你是专业的A股技术分析师，擅长综合研判各类技术指标。请严格返回JSON格式，不要有其他文字。"},
            ...
            buy_signal = advice_map.get(ai_result.get('operation_advice', '观望'), '观望')
            ...
            return {
                ...
                'target_price': ai_result.get('target_price', close),
                'stop_loss': ai_result.get('stop_loss', close * 0.95),
                'confidence_level': ai_result.get('confidence_level', 70),
            ...
            return {
                'buy_signal': '观望',
                'signal_score': 50,
                'analysis_summary': f'AI分析失败: {str(e)[:30]}',
                'target_price': close,
                'stop_loss': close * 0.95,
                'confidence_level': 50,
                'key_points': ['AI分析异常，仅供参考'],
            }

    # ========== StockHolmes 规则评分 ==========

    def analyze_stockholmes(self) -> dict:
        """StockHolmes规则评分：均线+MACD+RSI+乖离率+量能综合判断"""
        close = self._get('close')
        ma5 = self._get('sma5')
        ma10 = self._get('sma10')
        ma20 = self._get('sma20')
        ...
        vol_avg = self.df['volume'].tail(5).mean() if 'volume' in self.df.columns and self.df['volume'].tail(5).mean() > 0 else 1
        ...
        # 乖离率
        bias5 = round((close - ma5) / ma5 * 100, 2) if ma5 > 0 else 0
        if -3 <= bias5 <= 3:
            bias_status = f'乖离{bias5:+.1f}%'
            bias_score = 15
        elif bias5 > 5:
            bias_status = f'乖离{bias5:+.1f}%偏高'
            bias_score = 8
        elif bias5 < -5:
            bias_status = f'乖离{bias5:+.1f}%超跌'
            bias_score = 12
        else:
            bias_status = f'乖离{bias5:+.1f}%'
            bias_score = 12
        ...
        # 量能
        if vol_ratio > 1.5:
            vol_status = f'量比{vol_ratio}放量'
            vol_score = 15
        elif vol_ratio < 0.5:
            vol_status = f'量比{vol_ratio}缩量'
            vol_score = 8
        else:
            vol_status = f'量比{vol_ratio}正常'
            vol_score = 12

        rule_score = ma_score + macd_score + rsi_score + bias_score + vol_score
        ...
        signal_reasons = [ma_status, macd_status, rsi_status, bias_status, vol_status]
        risk_factors = []
        if rsi > 70:
            risk_factors.append('RSI超买，警惕回调')
        if close > ma5 * 1.05:
            risk_factors.append('乖离率偏大，有回落风险')
        if vol_ratio < 0.5:
            risk_factors.append('量能萎缩，资金观望')
        if dif < dea and dif > 0:
            risk_factors.append('MACD高位死叉风险')
        if not risk_factors:
            risk_factors.append('当前无明显风险')
    ...
    def generate_html(self, save_path: str, fund_flow_data: dict = None, stock_name: str = ""):
        indicators = [
            self.interpret_macd(),
            self.interpret_rsi(),
            self.interpret_ma(),
        ...
        sym = stock_name if stock_name else (self.symbol or "未知股票")
        sym_code = self.symbol or "未知股票"
...
<div class="card">
    <h2>技术指标解读</h2>
    {''.join(f'<div class="indicator-row"><span class="ind-name">{ind["name"]}</span><span class="ind-value">{ind["value"]}</span><span class="ind-status status-{ind["status_color"]}">{ind["status"]}</span><span class="ind-desc">{ind["desc"]}</span></div>' for ind in indicators)}
</div>
...
<div class="card">
    <h2>近期交易信号</h2>
    {"".join(f'<div class="signal-item"><span class="signal-date">{s["date"]}</span><span class="signal-type">{s["icon"]} {s["type"]}</span><span class="signal-desc">{s["desc"]}</span></div>' for s in signals) if signals else '<div style="color:#999;padding:12px">近期无明显交易信号</div>'}
</div>
...
<div class="footer">
    ...
    def generate_html_dual(self, save_path: str, fund_flow_data: dict = None, stock_name: str = "", stockholmes: dict = None, ai_analysis: dict = None):
        indicators = [
            self.interpret_macd(),
            self.interpret_rsi(),
            self.interpret_ma(),
        ...
        sh_score = stockholmes['signal_score'] if stockholmes else 50
        ai_score = ai_analysis['signal_score'] if ai_analysis else 50
        sh_signal = stockholmes['buy_signal'] if stockholmes else '观望'
        ai_signal = ai_analysis['buy_signal'] if ai_analysis else '观望'
        sh_reasons = stockholmes['signal_reasons'] if stockholmes else []
        ai_summary = ai_analysis.get('analysis_summary', '') if ai_analysis else ''
        ai_conf = ai_analysis.get('confidence_level', 0) if ai_analysis else 0
...
.dual-box .score-circle {{ margin-bottom: 8px; }}
.dual-box .signal {{ font-size: 14px; font-weight: 600; margin-top: 8px; }}
.dual-box .reasons {{ font-size: 12px; color: #666; margin-top: 8px; text-align: left; line-height: 1.8; }}
.dual-box .reasons::before {{ content: '▸ '; color: #999; }}
...
<div class="header">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-item">
            ...
            <div class="stat-value">{ma5:.2f}</div>
        </div>
        <div class="stat-item">
            <div class="stat-label">MA10</div>
            <div class="stat-value">{ma10:.2f}</div>
        </div>
        <div class="stat-item">
            <div class="stat-label">MA20</div>
            <div class="stat-value">{ma20:.2f}</div>
        </div>
        <div class="stat-item">
            <div class="stat-label">MA60</div>
            <div class="stat-value">{ma60:.2f}</div>
        </div>
    </div>
</div>
...
<div class="card score-section">
    ...
    <div class="dual-score">
        <div class="dual-box">
            ...
            <div class="reasons">
{'<br/>'.join(f'{r}' for r in sh_reasons[:4])}
            </div>
        </div>
        <div class="dual-box">
